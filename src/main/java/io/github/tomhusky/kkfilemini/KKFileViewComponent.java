...
import io.github.tomhusky.kkfilemini.exception.KKFileMiniException;
import io.github.tomhusky.kkfilemini.service.FileHandlerService;
import io.github.tomhusky.kkfilemini.service.FilePreview;
import io.github.tomhusky.kkfilemini.service.FilePreviewFactory;
...
public class KKFileViewComponent {
    ...
    public File convertViewFile(File file) {
        ...
        if (!allowFile(fileAttribute.getSuffix())) {
            throw new KKFileMiniException("不支持的文件类型");
        }
    ...
    public void viewCacheFile(HttpServletResponse response, String fileNo) {
        ...
        if (cacheFile == null) {
            throw new KKFileMiniException("文件不存在");
        }
        ...
        try {
            ...
            throw new KKFileMiniException("预览失败");
        } finally {
    ...
    public void addFileToCache(File file, String fileNo) {
        ...
        if (existFile) {
            throw new KKFileMiniException("文件编号已经存在,请尝试其他编号");
        }
        FileAttribute fileAttribute = fileHandlerService.getFileAttribute(file.getName());
        FilePreview filePreview = previewFactory.get(fileAttribute);
        if (filePreview == null) {
            throw new KKFileMiniException("添加缓存失败");
        }
        ...
        if (handleToFile == null) {
            throw new KKFileMiniException("转换失败");
        }
    }
